...
purrr::map(x, ~clim_dist_monthly(.x))
```
...
`clim_dist_seasonal` does not do parallel processing internally. You can wrap it in `mclapply`.
This is expected. Its first argument is the iterator index.
Annual is included as a "season" in the outputs from this function.
There is a one-to-one correspondence between monthly and seasonal files. `files` is simply a list of all the files
created by `clim_dist_monthly`.

```{r cmip5_seasonal, eval = FALSE}
in_dir <- snapdef()$ar5dir_dist_monthly
files_monthly <- list.files(in_dir, pattern = ".rds$", full.names = TRUE, recursive = TRUE)
parallel::mclapply(seq_along(files), clim_dist_seasonal, files = files_monthly)
```

Like the monthly outputs, now you have seasonal (and annual) climate variable distributions.
...
in_dir2 <- snapdef()$ar5dir_dist_seasonal
files_seasonal <- list.files(in_dir, pattern = ".rds$", full.names = TRUE, recursive = TRUE)
parallel::mclapply(seq_along(files), clim_dist_seasonal, files = files)
clim_stats_ar5(files_monthly, type = "monthly")
clim_stats_ar5(files_seasonal, type = "seasonal")
```
